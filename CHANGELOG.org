...
Includes numerous performance and usability improvements.

*** Added
- Package =vterm=: for better experience over Emacs native terminals.
- Enable =which-key-mode= with some customizations.
...
- Package =auto-dark= and local configuration =yet-auto-dark-themes=.
- New optional constants: ~yet-gptel-backend~, ~yet-gptel-model~,
  ~yet-gptel-remove-citations~, ~yet-gptel-default-directive~,
  ~yet-display-time~.
- Preserve ~log-edit-comment-ring~ across Emacs sessions.
- ~pandoc~-based ~yet-markdown-to-org~ export command.
- Package =nix-mode=: a major mode for editing Nix expressions.
- Quit =*vc-git : %s*= window after successful ~vc-git--pushpull~ run.
- Function ~yet-list-colors-display~ to fix display of defined colors on macOS.
- New optional constant ~yet-bidi-display~.
- Package =ibuffer-vc= to group buffers by VCS.
- Preserve ~kill-ring~ between Emacs sessions.
- New optional constant ~yet-safe-local-variable-values~.
- New optional constant ~yet-publishing-projects~ to automate loading
  project-specific org-mode publishing settings.

*** Changed
- Keybinding =C-c t s= now starts =vterm= instead of =ansi-term=.
- =gptel= post-response behavior.
- Set =gptel-log-level= to =info=.
- Enable =pdf-view-midnight-minor-mode= only if enabled theme is dark.
- =markdown-mode=: auto-detect markdown processor.
- Explicitly set =htmlize-html-charset= to "utf-8".
- Disable =htmlize-convert-nonascii-to-entities= (e.g., do not encode Cyrillic
  characters).
- Package =rainbow-mode= for colors preview replaced by =colorful-mode=.
- Restructure =ibuffer= settings.
- Performance related adjustments.
- Display list of buffers to kill before killing project buffers.
- Use ~yet-safe-local-variable-values~ to set ~safe-local-variable-values~.
- Enable ~org-highlight-latex-and-related~.
